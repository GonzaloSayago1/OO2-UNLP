...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                this.displayReceivedMessage(inputLine, clientSocket);//hook opcional
                
                if (this.shouldCloseSession(inputLine)) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);//hook obligatorio
            }
            this.displayConnectionClosed(clientSocket);//hook opcional
        } catch (IOException e) {
    ...
    protected boolean shouldCloseSession(String inputLine) 
    {
        return inputLine.equalsIgnoreCase(this.sessionCloseCommand());
    }
    
    protected String sessionCloseCommand() {
        return ""; // Comportamiento por defecto del framework, puedo sobreesbribir en subclases
    }
    
    protected void displayReceivedMessage(String inputLine, Socket clientSocket) {
        System.out.println("Received message: " + inputLine + " from "
            + clientSocket.getInetAddress().getHostAddress() + ":" + clientSocket.getPort());
    }
    
    protected void displayConnectionClosed(Socket clientSocket) {
